...
function getAuthorization(cburl,callback){
	...
	request(url+"?"+querystring.stringify(params),'get',function(d){
		var r=JSON.parse(d);
...
function apis(apiurl,callback,param,method,data){
	...
	request(apiurl+"?"+querystring.stringify(params),method,function(d){
...
function copyFile(callback,param){
     ...
     */
function uploadFile(callback,param,param2){
...
//请求连接
function request(uri,method,callback,data,headers)
{
	var opts = {};
	data = data || "";
	method = method || "GET";
