...
(function(){
	...
	function _style(node,params){
		...
		__each(params,function(v,k,a){
			if(_in_array(k,[C.LEFT,C.RIGHT,C.BOTTOM,C.TOP,C.WIDTH,C.HEIGHT,C.borderWIDTH,
			                C.borderRADIUS,C.paddingLEFT,C.paddingTOP,C.paddingRIGHT,C.paddingBOTTOM])){
				a[k]=v+"px";
			}
		},[DT.NUMBER]);
		if(_isDojo){
	...
	function _createClose(o){
		if(o.isClose){
			var closeNode=$dr.create($dr.C.DIV,{style:{
				position:$dr.C.ABSOLUTE,
				top:-7,
				right:3,
				width : 15,
				height : 15,
				background : $dr.COLOR.GRAY_BG,
				borderRadius : o.borderRadius,
				borderStyle:$dr.C.SOLID,
				borderWidth:1,
				borderColor:$dr.COLOR.GRAY
			}},node);
	...
	function _create(tag,params,pnode){
		if(tag===C.PATH){
			...
			if(_isDojo){
				dojo.byId(data.nBodysvgGroupId).appendChild(node);
			}else{
			...
			if(_isDojo){
				...
				if(pnode){
					...
					dojo.byId(data.nBodyId).appendChild(node);
				}
			}else{
				var node=$('<'+tag+'></'+tag+'>')[0];
				if(params){
	...
	function _drawShip(o){
		if(!o.n){
			...
			var right=3;
			if(o.isClose){
				o.nShipClose=$dr.create($dr.C.DIV,{
					className:"close",
					style:{
						position:$dr.C.ABSOLUTE,
						top:-7,
			...
			if(o.isEdit){
				o.nShipEdit=$dr.create($dr.C.DIV,{
					className:"edit",
					style:{
						position:$dr.C.ABSOLUTE,
						top:-7,
	...
	function main(node,params,_shiptypes){
		...
		var a=[];
		a.push("<div style='position: absolute;'>");
	...
	designer.getCustomRelation=getCustomRelation;
	window.$dr=designer;
})();