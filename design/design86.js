(function(){
	...
	function __each(a,f,p){
		...
		function _inf(v,k,a,p){
			p.push(k);
			var t=_get_class(v);
			if(_in_array(t,[C.STRING,C.BOOLEAN,C.NUMBER,C.HTMLDivElement,C.SVGPathElement])){
				f(v,k,a,t,p);
			}else{
				__each(v,f,p);
			}
	...
	function _style(node,params){
		var params=params||{};
		__each(params,function(v,k,a,t){
			if(t===C.NUMBER&&_in_array(k,[C.LEFT,C.TOP,C.WIDTH,C.HEIGHT])){
				a[k]=v+"px";
			}
		});
		$(node).css(params);
	...
	function _drawElement(o){
		if(!o.n){
			...
			if(typeObj.isOnlyIcon){
				...
				_create("div",{style:{paddingLeft:paddingLeft,paddingTop:3,width:70},innerHTML:o.title},node);
			}
		}
		_style(o.n,{
			position: "absolute",
