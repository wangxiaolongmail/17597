...
var data={
	...
	function _drawElement(o){
		if(!o.n){
			...
			if(typeObj.isOnlyIcon){
				...
				var paddingLeft=2*data.linkPad+data.orignPoint.w;
	...
	function _drawElementShadow(link){
		if(!link.n){
			link.n=_create("div");
	...
	function main(node){
		_each(data,function(val,key){
			if(key.indexOf("nBody")>=0){
				data[key]=data.id+val;
			}
		});
		var a=[];
